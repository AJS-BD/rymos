...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!validatePhone(phone)) { setError("Please enter a valid Bangladeshi phone number."); return; }
    ...
    try {
      ...
      const { error: authError, data } = await supabase.auth.signUp({
        email,
        password,
        options: { data: { full_name: fullName.trim(), phone: phone.replace(/\s/g, "") } },
      });
      ...
      <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <p className="text-[var(--color-text-muted)] mt-2">Join RYmos to start shopping</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div>
              ...
              {password && (
                <div className="mt-2">
                  <div className="flex gap-1">{[1,2,3,4,5].map((l) => <div key={l} className={`h-1 flex-1 rounded-full ${l <= passwordStrength ? strengthColors[passwordStrength] : "bg-gray-200"}`} />)}</div>
                  <p className="text-xs text-[var(--color-text-muted)] mt-1">{strengthLabels[passwordStrength]}</p>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPassword ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Re-enter your password"
                  className={`w-full pl-4 pr-10 py-3 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:border-transparent ${confirmPassword && confirmPassword !== password ? "border-red-300" : "border-[var(--color-border)]"}`} />
                {confirmPassword && confirmPassword === password && <Check className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-green-500" />}
              </div>
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? <span className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <>Create Account <ArrowRight className="w-4 h-4" /></>}
            </button>
          </form>

          <p className="text-center text-sm text-[var(--color-text-muted)] mt-6">
            Already have an account? <Link href="/auth/login" className="text-[var(--color-primary)] font-medium hover:underline">Sign in</Link>
          </p>
        </div>
      </div>
    </main>
